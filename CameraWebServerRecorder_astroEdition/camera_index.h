...
Only the 2640 camera is implemented so far.
The full text of the html and java is here and sent from the esp32 to the web browser to make it easy to edit.
When done editing, paste in the gzip version, and change the call to send gzip.

Details here:

https://arduinodiy.wordpress.com/2020/03/09/modifying-the-cam32-webserver/

 decrpyt the html and java
https://gchq.github.io/CyberChef/#recipe=From_Hex('Auto')Gunzip()

 encrypt the html and java
...
<html>
    ...
    <body>
        <section class="main">
            <div id="logo">
                <label for="nav-toggle-cb" id="nav-toggle">&#9776;&nbsp;&nbsp;Toggle OV2640 settings (CameraWebServer by @espressif, recording by @jameszah at github)</label>
            </div>
            <div id="content">
                <div id="sidebar">
                    ...
                    <nav id="menu">
                        ...
                        <div style="margin-top: 8px;"><center><span style="font-weight: bold;">Video Recording</span></center></div>
                        <hr style="width:100%">

                         <div class="input-group" id="freespace-group">
                              <label for="freespace">SD Freespace (MB)</label>
                                <div class="text">
                                    <input id="freespace" type="text" size="7"  class="default-action">
                                </div>
                         </div>
                         
                         <textarea id="parameters" name="parameters" rows="1" cols="40">
                         </textarea>

                         
                        <div class="input-group" id="seglen-group">
                            <label for="seglen">Avi Segment Length</label>
                            <select id="seglen" class="default-action">                               
                                <option value="10">10 sec</option>
                                <option value="30">30 sec</option>
                                <option value="60">1 min</option>
                                <option value="120">2 min</option>
                                <option value="600">5 min</option>
                        ...
                        <div class="input-group" id="interval-group">
                            <label for="interval">Frames per Sec</label>
                            <select id="interval" class="default-action">                               
                                <option value="0">full speed</option>
                                <option value="50">20 frame per sec</option>
                                <option value="100">10 frame per sec</option>
                                <option value="200">5 frame per sec</option>
                                <option value="500">2 frame per sec</option>
                                <option value="1000">1 frame per sec</option>
                                <option value="2000">2 sec per frame</option>
                                <option value="5000">5 sec per frame</option>
                                <option value="10000">10 sec per frame</option>
                                <option value="60000">1 min per frame</option>
                            </select>
                        </div>    
                        
                        
                        <div class="input-group" id="speed-group">
                            <label for="speedup">Multipy Framerate</label>
                            <div class="range-min">1</div>
                            <input type="range" id="speedup" min="1" max="600" value="1" class="default-action">
                            <div class="range-max">600</div>
                        </div>
                                   
                        <section id="buttons">
                            <button id="start-record">Start Record</button>
                            <button id="stop-record" class="disabled" disabled="disabled">Stop Record</button>                            
                        </section>
...
document.addEventListener('DOMContentLoaded', function (event) {
  ...
  function getReg(reg, offset, mask, cb){
    ...
    fetchUrl(`${baseHost}/greg?reg=${reg}&mask=${mask}`, function(code, txt){
      ...
      if(code == 200){
        ...
        value = (value & mask) >> offset;
        txt = ''+value;
        //console.log('Get Reg', '-', value, '-', txt, '-', reg, offset, mask );
        //console.log('Get Reg', '0x'+reg.toString(16), offset, '0x'+mask.toString(16), '0x'+value.toString(16), '('+value+')');
  ...
  const startrecButton = document.getElementById('start-record')
  const stoprecButton = document.getElementById('stop-record')
  ...
  const updateParams = () => {
    parameters.value = 'Fr: ' + framesize.value + ' Q: ' + quality.value +  ' Int: ' + interval.value + ' X: ' + speedup.value + ' Len: ' + seglen.value 
  }
  ...
  startrecButton.onclick = () => {
    ...
    view.src = `${baseHost}/startrecord?now=${real}`
      stopStream()
      enable(stoprecButton)
      disable(startrecButton)
      hide(viewContainer)
    //startRecord()
    //view.src = `${baseHost}/capture?_cb=${Date.now()}`
  ...
  stoprecButton.onclick = () => {
    view.src = `${baseHost}/stoprecord`
    
      enable(startrecButton)
      disable(stoprecButton)
      hide(viewContainer)
    //startRecord()
    //view.src = `${baseHost}/capture?_cb=${Date.now()}`
    //show(viewContainer)
  ...
  //const idis = document.getElementById('idis')                                
  const framesize = document.getElementById('framesize')

  interval.onchange = () => {
    updateConfig(interval)
    const newspeed = 30 / (1000 / interval.value)
    speedup.value = newspeed
    //idis.value = interval.value
    //freespace.value = freespace
    updateConfig(speedup)
    updateParams()
  }
